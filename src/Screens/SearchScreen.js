...
const SearchScreen = () => {
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `https://meernn.herokuapp.com/api/products/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
        );
        dispatch({
          type: 'FETCH_SUCCESS',
          payload: data,
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const { data } = await axios.get(`https://meernn.herokuapp.com/api/products/categories`);
        setCategories(data);
      } catch (err) {
